 ...
 * ​Even with a lock-free design, if the Non-RT thread (low priority) holds a system-wide resource (like a memory bus or a mutex for logging) that the RT thread needs, we will miss your 5 ms window.
